...
void SlideParameter::setMidiValue(const uint ccValue_)
{
    ...
    // for bidirectional parameters, we center the midi values to the value 64
    // by increasing the lower bound of midi values to 1 instead of 0
    float lowBound = (min < 0.f) ? 1.f : 0.f;
    
    float newValue = mapValue((float)ccValue_, lowBound, 127.f, 0.f, 1.f);
